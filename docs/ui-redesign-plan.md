...
6. **채팅방별 독립 설정**
   - DB `rooms.context` JSON 컬럼 활용
   - `room_load` 시 ContextHandler 자동 적용
   - `set_context`/`get_context`에서 `room_id` 지원

7. **Export HTTP 엔드포인트**
   - `/api/export` (JSON/ZIP)
   - `/api/export/stream` (NDJSON 스트리밍)
   - `/api/export/md` (Markdown ZIP)
   - 날짜 필터, 범위 선택, JWT 인증 지원
...
1. **~~채팅방별 독립 설정 구현~~** ✅ **완료**
   - [x] 서버: `set_room_context`, `get_room_context` 액션 추가
   - [x] 서버: DB `rooms.context` 컬럼 활용
   - [x] 클라이언트: 채팅방 전환 시 설정 자동 로드

2. **~~Export HTTP 엔드포인트~~** ✅ **완료**
   - [x] 서버: `/api/export` GET 핸들러 구현
   - [x] 서버: ZIP 압축 지원 + NDJSON 스트리밍 + Markdown
   - [ ] 클라이언트: 백업 모달에서 HTTP 다운로드 사용 (미완료)
...
1. ✅ **계획서 업데이트** (완료)
2. ✅ **채팅방별 독립 설정 구현** (완료)
3. ✅ **Export HTTP 엔드포인트** (서버 완료, 클라이언트 연동 필요)
4. ⬜ **프론트엔드 Export UI → HTTP 연동** (우선순위 🔴)
   - 백업 모달에서 HTTP 다운로드 사용
   - WebSocket fallback 제거

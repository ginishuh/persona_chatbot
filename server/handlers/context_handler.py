class ContextHandler:
    ...
    def build_system_prompt(self, history_text=""):
        ...
        # 요구사항: 'explicit(일반)' 수위를 한 단계 올려 기존 'enhanced' 수준으로 동작
        if adult_level == "explicit":
            prompt += """=== 콘텐츠 수위: 강화 ===
...
- **신음/의성어 혼합(상황 한정):** 친밀/성적 장면에서만 대사 안에 1~2회 가볍게 섞습니다(예: "하아…", "읏", "흣", "응"). 비성적 장면에서는 필요 시 짧은 숨 고르기/감탄(예: "헉", "하아") 정도로 대체하거나 생략합니다. 같은 표현의 과도한 반복 금지, 신음만 단독 줄 금지.

"""
        elif adult_level == "enhanced":
            prompt += """=== 콘텐츠 수위: 강화+ ===
- **명시어/Dirty Talk 빈도 상향:** 명시어와 Dirty Talk를 (문맥에 맞춰) 대사마다 1회 내외로 적극 활용. 과도한 파열형/나열식 Dirty Talk는 금지.
- **감각 디테일 강화:** 체위/접촉/리듬/체액/호흡/심박 등의 오감 디테일을 한층 세밀하게 기술. 문장 길이와 리듬에 변화를 주어 고조감을 형성.
- **신음/의성어(성적 장면 한정):** 성적 장면의 다수 대사에 짧은 신음을 자연스럽게 섞되(예: "하앙…", "읏", "후읏"), 같은 패턴을 반복하지 않습니다. 비성적 장면에서는 사용하지 않습니다.
- **관계/감정:** 소유/질투/집착/애정의 결을 대사·서술에 드러내되, 파열·관계 파괴의 수위는 Extreme에서만 허용.

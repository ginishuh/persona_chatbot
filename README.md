...
- 실시간 스트리밍 응답
- **대화 히스토리 관리**: 좌측 패널 슬라이더로 5~60턴(기본 30턴) 또는 무제한으로 조절, 서사 기록은 전체 대화 보존
...
- 좌측 `모드` 탭의 **🧠 맥락 길이** 슬라이더에서 5~60턴 범위를 선택하거나 `무제한`을 켜서 모든 대화를 전달할 수 있습니다. (기본 30턴)
- **♻️ 세션 유지** 토글을 켜면 Claude/Droid CLI 세션이 그대로 이어지고, 끄면 매 메시지마다 새로운 세션으로 stateless하게 동작합니다.
- 헤더의 세션 상태 배지(ON/OFF)와 **♻️ 세션** 버튼으로 언제든 세션 ID를 초기화할 수 있습니다.
- 서사(MD) 저장은 항상 전체 대화를 기준으로 하며, 히스토리 삭제는 헤더의 "🗑️ 대화 초기화" 버튼을 사용하세요.
...
**특징:**
- 프롬프트용 슬라이딩 윈도우: UI에서 지정한 턴 수(또는 무제한)에 맞춰 `deque`가 자동으로 오래된 메시지를 제거
- 별도 `full_history` 리스트를 유지해 서사 다운로드 시 전체 대화를 보존
- 히스토리 텍스트를 System Prompt에 포함하여 맥락 유지
- 마크다운 형식 서사 생성
...
| 세션 유지 토글 | 좌측 패널 → `모드` 탭 → **♻️ 세션 유지** | 기본 OFF. 켜면 Claude/Droid CLI 세션을 재사용해 내부 캐시/워크플로를 이어가고, 끄면 매 메시지마다 stateless하게 동작합니다. Gemini는 현재 stateless만 지원합니다. |
| 세션 리셋 | 헤더 우측 **♻️ 세션** 버튼 | 현재 WebSocket 연결에서 유지 중인 세션 ID를 초기화하여 다음 메시지부터 완전히 새 세션을 시작합니다. |
